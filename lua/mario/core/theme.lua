...
local config = {
    ...
    default = "sitruuna",
    state_file = vim.fn.stdpath("state") .. "/mario_theme.txt",
}
...
function M.select()
    vim.ui.select(config.themes, {
        ...
        format_item = function(item)
            if item == M.current() then return item .. " (current)" end
            return item
        end,
    }, function(choice)
...
function M.setup(opts)
    ...
    vim.api.nvim_create_user_command("Theme", function(cmd_opts)
        ...
        complete = function(arg_lead)
            return vim.tbl_filter(function(t) return t:find(arg_lead, 1, true) == 1 end, config.themes)
        end,
        desc = "Select or set theme",
    })

    vim.api.nvim_create_user_command("ThemeNext", function() M.cycle(1)  end, { desc = "Next theme" })
    vim.api.nvim_create_user_command("ThemePrev", function() M.cycle(-1) end, { desc = "Prev theme" })
