...
local function env_or(name, fallback)
    ...
    if value ~= nil and value ~= "" then return expand(value) end
...
local function normalize_workspace(workspace)
    if type(workspace) ~= "table" or type(workspace.path) ~= "string" then return nil end
    local normalized = vim.deepcopy(workspace)
    normalized.path = expand(normalized.path)
    if not is_dir(normalized.path) then return nil end
...
local function detect_vault_root()
    local candidates = { vim.api.nvim_buf_get_name(0), vim.uv.cwd() }
    for _, candidate in ipairs(candidates) do
        if type(candidate) == "string" and candidate ~= "" then
            local start = candidate
            if vim.fn.filereadable(start) == 1 then start = vim.fs.dirname(start) end
            local root = vim.fs.find(".obsidian", { path = start, upward = true, type = "directory", limit = 1 })[1]
...
local function find_daily_template(workspaces)
    for _, workspace in ipairs(workspaces) do
        if type(workspace.path) == "string" then
            local template = vim.fs.joinpath(expand(workspace.path), "Templates", "Daily Note.md")
            if vim.fn.filereadable(template) == 1 then return "Daily Note.md" end
        end
    end
    return nil
end
...
local function translate_obsidian_datetime_format(format)
    local tokens = {
        ...
        { "DD", "%d" }, { "D", "%-d" }, { "HH", "%H" }, { "H", "%-H" },
        { "hh", "%I" }, { "h", "%-I" }, { "mm", "%M" }, { "ss", "%S" },
        { "A", "%p" }, { "a", "__MARIO_OBSIDIAN_LOWER_AMPM__" },
    }
    local chunks = {}
    local i = 1
    while i <= #format do
        ...
        if char == "[" then
            ...
            if not matched then chunks[#chunks + 1] = char; i = i + 1 end
        end
    end
...
local function render_obsidian_datetime(format)
    ...
    return rendered:gsub("__MARIO_OBSIDIAN_LOWER_AMPM__", function()
        return os.date("%p"):lower()
    end)
end

function M.patch_template_substitutions()
    if vim.g.mario_obsidian_template_patch_applied then return end
    ...
    templates.substitute_template_variables = function(text, client, note)
        text = text:gsub("{{date:([^}]+)}}", function(fmt) return render_obsidian_datetime(fmt) end)
        text = text:gsub("{{time:([^}]+)}}", function(fmt) return render_obsidian_datetime(fmt) end)
        return original(text, client, note)
    end
    vim.g.mario_obsidian_template_patch_applied = true
end
...
function M.workspaces()
    if type(vim.g.obsidian_workspaces) == "table" and #vim.g.obsidian_workspaces > 0 then
        ...
        for _, workspace in ipairs(vim.g.obsidian_workspaces) do
            ...
            if normalized then workspaces[#workspaces + 1] = normalized end
        end
        if #workspaces > 0 then return workspaces end
    end

    local candidates = {
        { name = "northern-lights", path = env_or("OBSIDIAN_VAULT", "~/Northern.Lights") },
    }

    local existing = {}
    local seen = {}

    for _, workspace in ipairs(candidates) do
        local normalized = normalize_workspace(workspace)
    ...
    if detected_root and not seen[detected_root] then
        existing[#existing + 1] = { name = vim.fs.basename(detected_root), path = detected_root }
    end
...
function M.opts()
    ...
    return {
        ...
        note_id_func = function(title)
            ...
            if suffix == "" then suffix = tostring(os.time()) end
        ...
        frontmatter = {
            func = function(note)
                ...
                if note.metadata ~= nil and not vim.tbl_isempty(note.metadata) then
                    for key, value in pairs(note.metadata) do out[key] = value end
                end
                return out
            end,
        },
